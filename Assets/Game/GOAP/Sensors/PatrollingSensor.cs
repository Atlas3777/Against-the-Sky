...
namespace Game.GOAP.Sensors
{
    ...
    public class PatrollingSensor : LocalTargetSensorBase
    {
        public override ITarget Sense(IActionReceiver agent, IComponentReference references, ITarget existingTarget)
        {
            ...
            dataPatrolBehaviour.CalculateNextWaypoint();
            if (existingTarget is TransformTarget transformTarget)
                return transformTarget.SetTransform(dataPatrolBehaviour.CurrWaypoint.transform);
        
            return new TransformTarget(dataPatrolBehaviour.CurrWaypoint.transform);
        }
        // public override ITarget Sense(IActionReceiver agent, IComponentReference references, ITarget existingTarget)
        // {
