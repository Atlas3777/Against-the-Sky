...
#pragma warning restore CS0414
        ...
        private void ProjectileShot()
        {
            ...
            foreach (var p in firePoint)
            {
                ...
                bullet.damage = damagePerBullet * multipliers.damageMultiplier;
        ...
        private void ReEnableMelee() => isMeleeReady = true;
        ...
        private void HandleRecoil()
        {
            ...
            float speed = (weapon == null) ? 10 : weapon.recoilRelaxSpeed * 1.5f;
            if (!InputManager.shooting || reloading || !PlayerStats.Controllable)
            {
                cameraPivot.GetChild(0).localRotation = Quaternion.Lerp(cameraPivot.GetChild(0).localRotation, Quaternion.Euler(Vector3.zero), speed * Time.deltaTime);
            ...
            if (InputManager.shooting)
            {
                ...
                Quaternion recoilAdjustment = Quaternion.Euler(
            new Vector3(
                -weapon.recoilY.Evaluate(evaluationProgress) * yamount * .5f,
                -weapon.recoilX.Evaluate(evaluationProgressX) * xamount * .5f,
                0
            )
        );

                // Combine the current rotation with the recoil adjustment
                cameraPivot.GetChild(0).localRotation = Quaternion.Lerp(
                    cameraPivot.GetChild(0).localRotation,
                    cameraPivot.GetChild(0).localRotation * recoilAdjustment,
                    10 * Time.deltaTime
                );
            }
        }
