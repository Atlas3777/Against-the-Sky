...
namespace cowsins
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
       ...
       "       if false: Speed while shooting = walkSpeed")]
        ...
        private readonly float frictionThreshold = 0.01f;
        ...
        public void HandleVelocities()
        {
            ...
            if ((InputManager.sprinting || autoRun) && enoughStaminaToRun)
            {
                ...
                if (shouldRun && (canRunBackwards || Vector3.Dot(orientation.forward, rb.linearVelocity) > 0) && (canRunSideways || InputManager.x == 0 && InputManager.y != 0))
                {
                    ...
                    {
                        if(currentSpeed != runSpeed && !wallRunning) cameraFOVManager.SetFOV(runningFOV);
            ...
            {
                if (currentSpeed != walkSpeed && !wallRunning) cameraFOVManager.SetFOV(normalFOV); 
        ...
        private void Stamina()
        {
            ...
            if (!usesStamina || playerStatistics.IsDead || !PlayerStats.Controllable) return;
        ...
        public void Movement(bool move)
        {
            ...
            if (IsPlayerOnSlope())
            {
                rb.AddForce(GetSlopeDirection() * movementMultipliers);
                rb.useGravity = false;
                if (rb.linearVelocity.y > 0) rb.AddForce(Vector3.down * 70); 
            }
            else
            {
                Vector3 moveDirection = (orientation.transform.forward * InputManager.y + orientation.transform.right * InputManager.x).normalized;
        ...
        public void Jump()
        {
            ...
            if (doubleJumpResetsFallDamage) playerStatistics.height = transform.position.y;
        ...
        private void FrictionForce(float x, float y, Vector2 mag)
        {
            ...
            {
                rb.AddForce(acceleration * orientation.transform.right * Time.deltaTime * -mag.x * friction);
            }
            if (Math.Abs(mag.y) > frictionThreshold && Math.Abs(y) < 0.05f || (mag.y < -frictionThreshold && y > 0) || (mag.y > frictionThreshold && y < 0))
            {
                rb.AddForce(acceleration * orientation.transform.forward * Time.deltaTime * -mag.y * friction);
            }
        }
        ...
        public Vector2 FindVelRelativeToLook()
        {
            ...
            Vector3 localVel = Quaternion.Euler(0, -orientation.eulerAngles.y, 0) * rb.linearVelocity;
        ...
        void GetAllReferences()
        {
            ...
            playerStatistics = GetComponent<PlayerStats>();
        ...
        {
            if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, groundCheckDistance, whatIsGround))
            {
                Vector3 normal = hit.normal;

                // Check if the surface is considered a floor based on its normal
                if (IsFloor(normal))
                {
                    if (!grounded)
                    {
        ...
        public void HandleGrapple()
        {
            ...
            if (Vector3.Distance(joint.connectedAnchor, transform.position) > distanceToBreakGrapple)
            {
                if (grapplingHookMethod == GrapplingHookMethod.Linear)
                {
                    ...
                    float distance = Vector3.Distance(transform.position, joint.connectedAnchor);

                    // Scale force based on distance
                    float distanceMultiplier = Mathf.Clamp(distance, 1f, 10f);
                    Vector3 force = direction * grappleForce * distanceMultiplier;
        ...
        {
            if (!allowGrapple || !grappleEnabled || grappling || !PlayerStats.Controllable) return;
            RaycastHit hit;
        ...
        {
            // Check Jump
            if (PlayerStats.Controllable && allowJump && readyToJump && (enoughStaminaToJump && (grounded || canCoyote) || wallRunning || jumpCount > 0 && maxJumps > 1 && enoughStaminaToJump))
                playerStates.ForceChangeState(playerStates._States.Jump());
        }
        public void SwitchToDashState()
        {
            // Check Dash
            if (PlayerStats.Controllable && canDash && (infiniteDashes || currentDashes > 0 && !infiniteDashes))
